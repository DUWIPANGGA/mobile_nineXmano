...
enum FirebaseNode { ITEN_USER, ITEN_SYSTEM }
...
class FirebaseDataService {
  ...
  static const String _databaseURL =
      "https://mano-database-ba7bb-default-rtdb.firebaseio.com/";
  ...
  Future<bool> checkAnimationExists(AnimationModel animation) async {
    try {
      ...
      String email = config?.email.split('.').first ?? 'CC';
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        for (final entry in data.entries) {
          ...
          if (animationData is Map &&
              animationData['identifier'] == expectedIdentifier) {
  ...
  Future<bool> saveLocalAnimationToCloud(AnimationModel animation) async {
    try {
      ...
      final String jsonStringValue = (jsonEncode(
    ...
    switch (node) {
      ...
      case FirebaseNode.ITEN_USER:
        if (subPath.isEmpty) {
          ...
          final userAnimations = await _prefsService.getApiUserAnimations();
          final animation = userAnimations?.animations.firstWhere(
            (anim) => anim.name == subPath,
          );
  ...
  Future<void> _autoSaveToPreferences(
    FirebaseNode node,
    String subPath,
    dynamic data,
  ) async {
  ...
  Future<void> testAutoSaveFunctionality() async {
    try {
      ...
      if (cachedAnimations != null && cachedAnimations.isNotEmpty) {
        ...
        final specificAnimation = await getData(
          FirebaseNode.ITEN_USER,
          firstAnimationName,
        );
        ...
        final isSelected = userSelections.any(
          (anim) => anim.name == firstAnimationName,
        );
  ...
  Future<String?> createData(
    ...
    try {
      ...
      final path = _getNodePath(node, subPath);
      final response = await _dio.post('$path.json', data: jsonEncode(data));
  ...
  Future<bool> updateData(
    ...
    try {
      ...
      final path = _getNodePath(node, subPath);
      final response = await _dio.put('$path.json', data: jsonEncode(data));
  ...
  Future<bool> batchUpdate(Map<String, dynamic> updates) async {
    try {
      ...
      final response = await _dio.patch('.json', data: jsonEncode(updates));
  ...
  Future<bool> createUserAnimation(AnimationModel animation) async {
    try {
      final updates = {animation.name: animation.toList()};
  ...
  Future<List<AnimationModel>> getUserAnimationsByChannel(
    int channelCount,
  ) async {
    try {
      final collection = await getUserAnimations();
