...
static final List<Map<String, dynamic>> _defaultAnimationsData = [
  ...
  Future<void> _initializeApp() async {
    try {
      ...
      _selectedWelcomeAnimation =
          _defaultAnimationsData[safeIndex]['name'] as String;

      print(
        "Animasi yang dipilih: $_selectedWelcomeAnimation, UI Index: $safeIndex, Device ID: ${safeIndex + 1}",
      );
  ...
  void _initializeData() {
    ...
    _ssidController.text = config?.ssid ?? 'COLEGA VARIASI';
    ...
    return Container(
      ...
      child: DropdownButton<int>(
        ...
        onChanged: (int? newIndex) {
          if (newIndex != null &&
              newIndex >= 0 &&
              newIndex < _defaultAnimationsData.length) {
            print('🔄 Dropdown changed to index: $newIndex'); // Debug
            setState(() {
              _selectedAnimationIndex = newIndex;
              _selectedWelcomeAnimation =
                  _defaultAnimationsData[newIndex]['name'] as String;
            });
    ...
    if (parts.length >= 20) {
      setState(() {
        ...
        if (licenseLevel > 0) {
          ...
          _selectedChannel = deviceChannel <= _maxChannels
              ? deviceChannel
              : _maxChannels;
        } else {
  ...
  void _updateChannel() {
    ...
    if (licenseLevel == 0) {
      _showSnackbar(
        'Tidak dapat mengubah channel: Aktifkan lisensi terlebih dahulu',
      );
      return;
    }
  ...
  Widget _buildChannelActionButton() {
    final licenseLevel = int.tryParse(_licenseLevel) ?? 0;
    final isLicenseValid = licenseLevel > 0;
    final isChannelValid =
        _selectedChannel > 0 && _selectedChannel <= _maxChannels;

    return _buildActionButton(
      text: isLicenseValid
          ? 'UBAH CHANNEL'
          : 'AKTIFKAN LISENSI TERLEBIH DAHULU',
      onPressed: isLicenseValid && isChannelValid ? _updateChannel : null,
      enabled:
          widget.socketService.isConnected && isLicenseValid && isChannelValid,
      backgroundColor: isLicenseValid
          ? AppColors.neonGreen
          : AppColors.darkGrey,
      foregroundColor: isLicenseValid
  ...
  void _updateEmail() {
    if (_emailController.text.isNotEmpty) {
      ...
      _showSnackbar('Mengirim nama: ${_emailController.text}');
    }
  }
  ...
  void _updateWelcomeSettings() {
    ...
    if (_selectedAnimationIndex < 0 ||
        _selectedAnimationIndex >= _defaultAnimationsData.length) {
      _showSnackbar('Animasi tidak valid');
      return;
    }

    final deviceAnimationId =
        _selectedAnimationIndex + 1; // Convert to 1-based ID
          ...
          children: [
            ...
            _buildSection(
              ...
              children: [
                ...
                if (_licenseLevel.isNotEmpty &&
                    int.tryParse(_licenseLevel)! > 0) ...[
                  const SizedBox(height: 12),
                  _buildSettingRow(
                    title: 'Ubah Channel',
